...
if (isset($_POST['send'])){
    ...
    if (empty($username)){
        echo '<div class="alert alert-danger text-center" role="alert"><b>الرجاء ادخال اسم المستخدم </b></div>';
    }elseif (empty($email)) {
        echo '<div class="alert alert-danger text-center" role="alert"><b>الرجاء ادخال البريد الالكتروني </b></div>';
    }elseif (!filter_var($_POST['email'],FILTER_VALIDATE_EMAIL)){  /*هنا استخدمت فلتر للايميل عشان أتأكد ان كان المدخل ايميل صحيح ولا لا*/
        echo '<div class="alert alert-danger text-center" role="alert"><b>الرجاء ادخال بريد الكتروني صحيح </b></div>';
    }else {
        ...
        if ($count > 0) {
            if ($username == $row_select->username AND $email == $row_select->email){
                                    ...
                                    if ($image_size <= 3000000) {
                                        ...
                                        if (move_uploaded_file($image_temp, $image_dir)) { /*هنا بشيك ان كان رفع الصوره يكمل اللي جوا if بقا*/
                                            ...
                                            if (isset($stmnt)) { /*هنا بشيك لو جمله $stmnt بتاعتي اتنفذت يكمل اللي بعد كدا*/
                                                echo '<div class="alert alert-success text-center" role="alert"><b> تم تحديث بيانات العضو بنجاح , جاري تحويلك لصفحه الأعضاء :) </b></div>';
                ...
                $count = $sql->rowCount();
                if ($count > 0){
                    echo '<div class="alert alert-danger text-center" role="alert"><b>اسم المستخدم مسجل بالفعل</b></div>';
                }else{
                    if ($_POST['password'] != '' OR $_POST['con_password'] != ''){
                        if ($_POST['password'] != $_POST['con_password']){
                            echo '<div class="alert alert-danger text-center" role="alert"><b>عفوا كلمه المرور غير متطابقه</b></div>';
                ...
                $count = $sql->rowCount();
                if ($count > 0){
                    echo '<div class="alert alert-danger text-center" role="alert"><b> البريد الالكتروني مسجل بالفعل</b></div>';
                }else{
                    if ($_POST['password'] != '' OR $_POST['con_password'] != ''){
                        if ($_POST['password'] != $_POST['con_password']){
                            echo '<div class="alert alert-danger text-center" role="alert"><b>عفوا كلمه المرور غير متطابقه</b></div>';
        ...
        <div class="panel-body">
            <form action="" method="post" class="form-horizontal col-md-9" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <textarea name="about" class="form-control" id="about_you" rows="4"><?php echo $row_select->about_user; ?></textarea>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <select name="role" class="form-control" id="role">
                            <option value="user" <?php if ($row_select->role == 'user'){echo 'selected';} ?>>عضو</option>
                            <option value="admin" <?php if ($row_select->role == 'admin'){echo 'selected';} ?>>مدير</option>
                            <option value="writer" <?php if ($row_select->role == 'writer'){echo 'selected';} ?>>كاتب</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
